require(['jquery', 'jquery/ui'], function($){
	...
	jQuery("input[name=loginwithemail]").change(function(){
		if (jQuery(this).is(":checked")) {
			jQuery(".ajaxloginphtml").show();
		} else {
			jQuery(".ajaxloginphtml").hide();
		}
	});

	$(document).on('click','.authentication-wrapper button.action-auth-toggle',function(){
		jQuery(".checkout-index-index .ajax-login-form").css('display','block');
		jQuery(".checkout-index-index .modal-custom.authentication-dropdown.custom-slide").css('display','none');
	});

	$(document).on('click','.checkout-index-index .gotoregister',function(e){
		jQuery(".checkout-index-index .ajax-login-form").css('display','none');
		jQuery(".checkout-index-index .ajax-register-form").css('display','block');
	});
	
	$(document).on('click','.checkout-index-index .modal .closediv',function(e){
		jQuery(".checkout-index-index .ajax-login-form").css('display','none');
		jQuery(".checkout-index-index .ajax-register-form").css('display','none');
		jQuery(".checkout-index-index .ajax-forgot-form").css('display','none');
		jQuery(".checkout-index-index .regcontollererror").css('display','none');
	});
	
	jQuery(".checkout-index-index .loginverifyotp").click(function(e) {
		var mobile = jQuery("#loginotpmob").val();
		var countrycode = jQuery("#country-code-login").val();
		...
		jQuery.ajax({
			 url: url,
			 type:'GET',
			 data:{mobile:mobile,otp:otp,countrycode:countrycode},
	...
	jQuery(".gotoregister").click(function() {
		jQuery(".loginwithemail .closediv").click();
		jQuery(".registerlink").click();
		jQuery(".mobileExist").hide();
		jQuery(".ajax-register-form #mobileget").val('');
	});
	jQuery(".gotologin").click(function() {
		jQuery(".ajax-register-form .closediv").click();
		jQuery(".ajaxlogin-login").click();
		jQuery(".loginsendotperror").hide();
		jQuery(".loginwithemail #loginotpmob").val('');
	});
   ...
   jQuery(".ajaxlogin-login").click(function(e) {
	...
	jQuery("#customerloginsubmit").click(function(e) {
	   ...
	   jQuery("#customer-login-please-wait").css('display','block');
   	   jQuery(".cuslogsub").css('display','none');
	   //  jQuery("#loginsubmit").css('display','none');
	   jQuery(".emailpasswrong").css('display','none');
	   jQuery.ajax({
                ...
                success: function(response) {
					  if(response.error){
						  ...
						  jQuery('#customer-login-please-wait').css('display','none');
				     	  jQuery(".cuslogsub").css('display','block');
	 					  jQuery("#loginsubmit").css('display','block');
							return;
						}
...
// 	});
    ...
    jQuery(".verifyotp").click(function(e) {
		...
		var countrycode = jQuery("#country-code-register").val();
		...
		jQuery.ajax({
		   ...
		   type: 'GET',
		   data:{otp:otp,mobile:mobile,countrycode:countrycode},
		   success: function(data) {
	...
	jQuery(".sendotp").click(function(e) {
		...
		jQuery.ajax({
		   ...
		   success: function(data) {
		 	     ...
		 	     jQuery("#reg-sms-please-wait").css('display','none');
			   if(data == 'true'){
				    jQuery("#createmobile").val(countrycode+mobile);
		 	    ...
		 	    jQuery("#reg-sms-please-wait").css('display','none');
			   	if(data == 'true'){
					...
					jQuery(".sending").css('display','none');
			   		...
			   		if(data != 'error') {
			   			jQuery(".mobileotpsenderror").html(data);
			   		}
				  	jQuery(".mobileotpsenderror").css('display','block');
			   }
	...
	jQuery(".forgotsendotp").click(function(e) {
		...
		var countrycode = jQuery("#country-code-forgot").val();
	...
	jQuery(".forgotverifyotp").click(function(e) {
		...
		jQuery("#forgot-sms-verify-please-wait").css('display','block');
			...
			jQuery.ajax({
			   url: url,
			   type:'GET',
			   data:{mobile:mobile,otp:forgototp,countrycode:countrycode},
			   success: function(data) {
	...
	jQuery(".updatepassbtn").click(function(e) {
		...
		if(newpassotp == newpassconrmotp){
			jQuery.ajax({
				   url: url,
				   type:'GET',
				   data:{mobile:mobile,otp:forgototp,newpass:newpassotp,confirmpass:newpassconrmotp,countrycode:countrycode},
				   success: function(data) {
