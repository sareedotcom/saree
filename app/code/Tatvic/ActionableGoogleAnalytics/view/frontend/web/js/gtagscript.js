define(['jquery','gtag_script'], function($,gtag_script){
    "use strict";
    return function gtag_script() {
        this.custom_dimentions = function () {

            gtag('config', tvc_UA_ID, {
                'custom_map': {
            ...
            if(tvc_link_attr === 1){
                ...
                gtag('config', tvc_UA_ID, { 'link_attribution': true,'send_page_view' : false ,'non_interaction': true});
            }
            if (tvc_user_id !== '') {
                gtag('config', tvc_UA_ID, { 'user_id': tvc_user_id,'send_page_view' : false ,'non_interaction': true});
            }

            if(tvc_ads_feature === 1){
                gtag('config', tvc_UA_ID, { 'allow_ad_personalization_signals': false,'send_page_view' : false ,'non_interaction': true});
            }

            if(tvc_ip === 1){
                gtag('config', tvc_UA_ID, { 'anonymize_ip': true,'send_page_view' : false ,'non_interaction': true});
            }
        }
