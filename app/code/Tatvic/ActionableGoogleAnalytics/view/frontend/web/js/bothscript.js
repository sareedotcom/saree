    ...
    return function both_script() {
        this.custom_dimentions = function () {
            ...
            if(tvc_link_attr === 1){
                ...
                gtag('config', tvc_ga4_both, { 'link_attribution': true,'send_page_view' : false,'non_interaction': true });
            }

            if (tvc_user_id !== '') {
                gtag('config', tvc_ga4_both, { 'user_id': tvc_user_id,'send_page_view' : false,'non_interaction': true });
            }

            if(tvc_ads_feature === 1){
                gtag('config', tvc_ga4_both, { 'allow_ad_personalization_signals': false,'send_page_view' : false,'non_interaction': true });
            }
            if(tvc_ip == 1){
                gtag('config', tvc_ga4_both, { 'anonymize_ip': true,'send_page_view' : false,'non_interaction': true });
            }
        }
