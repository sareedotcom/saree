 ...
 */
namespace Logicrays\Customization\Controller\Adminhtml\Order\Creditmemo;
    ...
    public function execute()
    {
        ...
        try {
            ...
            if ($creditmemo) {
                ...
                $order = $this->_order->load($this->getRequest()->getParam('order_id'));

                if($order->getBaseTotalRefunded() >= $order->getSubtotal()){
                    $order->setState($order->getState())->setStatus("refunded");
                }
                else{
                    $order->setState($order->getState())->setStatus("partial_refund");
                }
                $order->save();
