...
namespace Logicrays\UpdateOrder\Model\Order;
...
class Item extends \MageWorx\OrderEditor\Model\Order\Item
{
    ...
    protected function updateItemData()
    {
        ...
        // Lr item status
        if (isset($this->newParams['lr_item_status']) && isset($this->newParams['is_status_update'])) {
            $this->setLrItemStatus($this->newParams['lr_item_status']);

            $username = $this->authSession->getUser()->getUsername();
            $append = " <b>(By : ".$username.")</b>";
            $this->_order->addStatusHistoryComment('sku: '.$this->getSku().' Item status is change to '.$this->newParams['lr_item_status']." ".$append);
        }
